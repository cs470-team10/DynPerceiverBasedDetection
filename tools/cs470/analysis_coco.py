    ...
    if (len(annotations) == 1):
        ...
        if (len(draw_bbox_indexes) > 0):
            for draw_bbox_index in draw_bbox_indexes:
                ...
                exit_stage, estimated = model.analysis_threshold(out, T)
                draw_bbox(coco, image, f"{output_dir}/images/{file_name(config[draw_bbox_index], 'image', '')}", set_name, image_id, exit_stage, estimated, annotations)
...
def analysis(config):
    ...
    if (len(draw_bbox_indexes) > 0):
        ...
        for draw_bbox_index in draw_bbox_indexes:
            ...
            print(formatting_config_entry(config_entry, "images/" + file_name(config[draw_bbox_index], 'image', '') + "/*"))
    ...
    for config_entry in tqdm(config, desc=f'Drawing graphs'):
        draw_graph(config_entry)
        
    print(f"Total analyzed images: {len(image_ids)}/{len(coco.getImgIds())}")
